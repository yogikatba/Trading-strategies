...
import './App.css';
      ...
      class Bull_spread_using_call extends React.Component {
  constructor(props){
    super(props);
    this.state = {
      option: 'put',
  ...
  profit(c){
        console.log(c);
        if(c <= this.state.K1){
            this.setState({P : Number(this.state.C1)-Number(this.state.C2)});
            return (Number(this.state.C1)-Number(this.state.C2));
        } 
       else if( (this.state.K1  < c && c <= this.state.K2))
            {
                const name=c-Number(this.state.K1);
                const p2= Number(this.state.C1)-Number(this.state.C2) + name;
                //console.log(p2);
                this.setState({P : p2});
                return p2;
            }
        ...
        /*else if( (this.state.K2  < c && c <= this.state.K3))
       ...
       else if( ( c > this.state.K2))
            {
                ...
                const p4=Number(this.state.C1)-Number(this.state.C2) +name;
  ...
  render() {
    return (
        ...
        <h1 style={{textAlign:'center',fontWeight:'bold',fontSize:30,color:'blue'}}>Bull Spread using call</h1>
      
        
        <div className="infoitem">
        <ul>
         <li>an investor who enters into a bull spread is hoping that the stock price will increase.</li>
        
        <li>investor is using call options to form bull spread.</li>
        
        <li>The investor buys a call with a low strike price and sells a call with a high strike price.</li>
        
        <li>Lets suppose that we will buy the call option at strike price K1 which is higher than the strike price K1.We will sell call option at strike price K2.</li>
        
        <li>So when the strike price will be less than K1 none of the option will executed .</li>
         
        <li>when the strike price will be in between K1 and K2 ,call option with K1 strike price will be executed but K2 will not we executed.</li>
        
        <li>when the strike price will be graeater than K2 both the option will be executed and total payoff will be K2-K1.</li>
        </ul>
...
export default Bull_spread_using_call